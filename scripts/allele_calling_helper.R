...
get_adjacent_x_with_non_max_y <- function(x, y, max_diff) {
  ...
  if (length(unique(adj_x$group)) == 1) {
    no_max_y <- which_not_max(adj_y) 
    adj_x_no_max_y <- select_subset(adj_x$index, no_max_y)
...
#'@param f OSIRIS tab-delimited file
#'@param s sample name
#'@param pullUp_dist1 base pair distance around off-scale peak to find small pull-up
#'@param max_pullup_rfu maximum RFU of small pull-up 
#'@return a list of integer index of pull-up fragment sizes
find_pull_up1 <- function(f, s, pullUp_dist1, max_pullup_rfu) {
  spl_dat <- get_sample_dat(f, s)
  frag_size <- spl_dat$size
  frag_rfu <- spl_dat$intensity
  ...
  for (i in seq_along(frag_size)) { # current colour channel
    for (j in seq_along(frag_size[[i]])) { # size in a channel
      for (k in seq_along(frag_size)[-i]) { # other colour channels
        
        if (length(frag_size[[i]]) == 0 || 
            length(frag_size[[k]]) == 0) {
          next
        }
        
        left_side = frag_size[[i]][j] - pullUp_dist1
        right_side = frag_size[[i]][j] + pullUp_dist1
        
        pos <- which(frag_size[[k]] >= left_side & 
                       frag_size[[k]] <= right_side &
                       frag_rfu[[k]] <= frag_rfu[[i]][j] &
                       frag_rfu[[k]] <= max_pullup_rfu)
...
remove_pull_up1 <- function(f, s, pullUp_dist1, max_pullup_rfu) {
  spl_dat <- get_sample_dat(f, s)
  frag_size <- spl_dat$size
  frag_rfu <- spl_dat$intensity
  
  pull_up_index <- find_pull_up1(f, s, pullUp_dist1, max_pullup_rfu)
...
#'@param max_pullup_rfu maximum RFU of small pull-up 
#'@return a data frame
remove_pull_up1_all <- function(f, ctrl, pullUp_dist1, max_pullup_rfu) {
  ctrl_sample <- find_ctrl(f, ctrl)
  sample <- f[!ctrl_sample,]
  ...
  for (i in seq_along(sample_name)) {
    ...
    out <- remove_pull_up1(f, current_sample, pullUp_dist1, max_pullup_rfu)
...
#'@param min_off_rfu minimum intensity to find off-scale peaks
#'@param ru_len repeat unit length, base pair distance to look for stutters
#'@param pullUp_dist2 base pair distance around off-scale peak to find high pull-up
#'@return a list of integer index of pull-up fragment sizes
find_pull_up2 <- function(f, s, min_off_rfu, ru_len, pullUp_dist2) {
  spl_dat <- get_sample_dat(f, s)
  frag_size <- spl_dat$size
  num_colours <- length(frag_size)
  ...
  for (i in seq_along(off_size)) { # off-scale channel, i = 1 to 4
    for (j in seq_along(off_size[[i]])) { # off-scale size in a channel
      for (k in seq_along(off_size)[-i]) { # not off-scale channel
        ...
        left_side = off_size[[i]][j] - pullUp_dist2
        right_side = off_size[[i]][j] + pullUp_dist2
...
#'@param pullUp_dist2 base pair distance around off-scale peak to find high pull-up
#'@return fragment size and intensity with pull-up removed
remove_pull_up2 <- function(f, s, min_off_rfu, ru_len, pullUp_dist2) {
  spl_dat <- get_sample_dat(f, s)
  frag_size <- spl_dat$size
  frag_rfu <- spl_dat$intensity
  
  pull_up_index <- find_pull_up2(f, s, min_off_rfu, ru_len, pullUp_dist2)
...
#'@param pullUp_dist2 base pair distance around off-scale peak to find high pull-up
#'@return a data frame
remove_pull_up2_all <- function(f, ctrl, min_off_rfu, ru_len, pullUp_dist2) {
  ctrl_sample <- find_ctrl(f, ctrl)
  sample <- f[!ctrl_sample,]
  sample_name <- unique(sample$File.Name)
  ...
  for (i in seq_along(sample_name)) {
    ...
    out <- remove_pull_up2(f, current_sample, min_off_rfu, ru_len, pullUp_dist2)
...
#'@param f OSIRIS tab-delimited file
#'@param ctrl patterns of control sample name
#'@param min_off_rfu minimum intensity to find off-scale peaks
#'@param split_dist maximum base pair distance to find off-scale split peaks
#'@return a data frame
...
#'@param x a numeric vector of fragment sizes
#'@param y a numeric vector of contamination sizes
#'@return x if no contamination or contaminated size replaced with letter "C"
label_contamination <- function(x, y) {
  if (all(is.na(y))) {
    out <- x
  } else {
    out <- replace(x, which(x %in% y), "C") |> num_to_chr()
  }
  return(out)
}
...
remove_contamination <- function(f, s, ctrl, cont_dist) {
  ...
  cont_size <- lapply(seq_along(cont_index)
                     , function(i) {
                       frag_size[[i]][ cont_index[[i]] ]
                     })
  
  frag_size2 <- lapply(seq_along(frag_size)
                       , function(i) {
                         label_contamination(frag_size[[i]]
                                             , cont_size[[i]])
                       })
  
  frag_rfu2 <- lapply(seq_along(frag_rfu)
                      , function(i) {
                        label_contamination(frag_rfu[[i]]
                                            , cont_size[[i]])
                      })
