...
ru_len <- 4 # repeat unit length, base pair to look for stutters
max_pullup_rfu <- 5000 # maximum RFU of small pull-up
pullUp_dist1 <- 0.5 # base pair around off-scale peak to find small pull-up
pullUp_dist2 <- 0.5 # base pair around off-scale peak to find high pull-up
split_dist <- 1.5 # base pair to find off-scale split peaks
A_dist <- 1.5 # base pair to find non-template addition, stutter
...
cont_dist <- 0.5 # base pair around peaks in control to be treated as contamination
...
# remove pull-up 
osiris_out2.1 <- lapply(osiris_out
                        , remove_pull_up1_all
                        , control
                        , pullUp_dist1
                        , max_pullup_rfu)

osiris_out2.2 <- lapply(osiris_out2.1
                        , remove_pull_up2_all
                        , control
                        , min_off_rfu
                        , ru_len
...
osiris_out3 <- lapply(osiris_out2.2
                      , get_mean_split_all
                      , control
                      , min_off_rfu
                      , split_dist)
